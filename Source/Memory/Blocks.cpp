...
namespace {
    ...
    private:
        ...
        [[nodiscard]] uintptr_t compute_base(uint32_t block) const noexcept {
            return m_start_address + (block << g_block_size_shl);
        }

        static uintptr_t block_align(uintptr_t in) noexcept {
        ...
        public:
            ...
            [[nodiscard]] bool remove_max_if(uintptr_t v) noexcept {
        ...
        private:
            ...
            bool single_rotate_left(avl_node *n_left) noexcept {
            ...
            bool single_rotate_right(avl_node *n_right) noexcept {
            ...
            bool double_rotate_left_right(avl_node *avl) noexcept {
                single_rotate_left(avl->left);
                return single_rotate_right(avl);
            }

            bool double_rotate_right_left(avl_node *avl) noexcept {
                single_rotate_right(avl->right);
                return single_rotate_left(avl);
            }

            bool try_balance(avl_node *avl) noexcept {
            ...
            void delete_leaf(avl_node *node) noexcept {
                const auto parent = node->parent;
                // update min-max tags
        ...
        void release_id(uint32_t id) noexcept {
            if (id + 1 == m_brk) {
                --m_brk;
...
namespace neworld::memory {
    uintptr_t fetch_block() noexcept { return block_host::instance().allocate(); }

    void return_block(uintptr_t block) noexcept { return block_host::instance().free(block); }
}